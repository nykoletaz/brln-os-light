...
export const getRebalanceHistory = (limit = 50) => request(`/api/rebalance/history?limit=${limit}`)
export const runRebalance = (payload: { channel_id: number; channel_point?: string; target_inbound_pct?: number }) =>
...
export const updateRebalanceChannelTarget = (payload: { channel_id: number; channel_point: string; target_inbound_pct: number }) =>
  request('/api/rebalance/channel/target', { method: 'POST', body: JSON.stringify(payload) })
export const updateRebalanceChannelAuto = (payload: { channel_id: number; auto_enabled: boolean }) =>
